...
function initFeatures(settings) {
    ...
    function genLocalForm () {
        var localForms = {
			...
			'kz' : '<div class="order_block"> <div class="prod_img_wrapper"> <img src="' + settings.form.img + '" width="300" class="prod_img">' + ((settings.form.price) ? '<div class="priceс"><span class="discountс">Тиімді!</span> <s><span class="js_old_price price_old">' + settings.priceMain * 2 + settings.currencyMain + '</span></s><br> <b class="new_price js_new_price price_main">' + settings.priceMain + settings.currencyMain + '</b> </div>' : '') + '</div> <center> <h3 style="text-align: center">Ұсыныс басқа оқырмандарға берілмейінше, жеңілдік алуға уақыт бар! <br>Жеңілдік келесіден кейін белсенді<br>болмайды:  <span class="time_remains" id="min"></span> : <span class="time_remains" id="sec"></span> </h3> </center> <form class="order_form" action="getform.php" method="POST"> <select class="country_select wheel_input" name="country">' + ((!!settings.formApi && settings.formApi.isNeeded) ? formApiCode : '</select>')  + ' <label for="name" class="data_label">Атыңызды енгізіңіз:</label><input class="wheel_input" type="text" name="name" required="" id="name" placeholder="Омар"><label for="phone" class="data_label">Телефонды енгізіңіз:</label> <input class="wheel_input" type="text" id="phone" name="phone" required="" placeholder="+7XXXXXXXXXXX">  <input type="hidden" name="landing" value="' + ((!!settings.formApi && settings.formApi.isNeeded) ? settings.formApi.landingValue : window.location.host) + '"> <button class="main-link" type="submit">50% жеңілдікпен сатып алыңыз!</button> </form> </div><center> <h2 class="time_remains_title">Бұл ұсыныс келесіге дейін жарамды: <span class="time_remains">' + outputDat(0, false) + '</span></h2> <br> </center>',
